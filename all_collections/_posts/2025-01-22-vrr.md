...
date_modified: 2026-03-25 12:00:00 +0200
categories: [Monitors]
permalink: /vrr/
---
...
**V-Sync**: With VRR on + V-Sync off, if a new frame becomes ready early while the previous frame is still being scanned out onto the display (from top to bottom), the remaining scanout will be replaced by the new frame, resulting in tearing. The same is true for a frametime spike where the old frame exceeds the maximum frame hold time (VBLANK interval) and repeats whilst a new frame becomes ready and replaces it during the scanout. When V-Sync is enabled in conjunction with VRR, the driver will adhere to the display scanout by holding the new frame until a new scanout, which ensures no tearing. This replaces default V-Sync buffered behavior and therefore the only added input lag is preventing the frame from being overwritten during the active scanout.

**FPS Limiter**: VRR only works inside a display's refresh rate range and is automatically disabled when the framerate exceeds the max refresh rate. Capping at the display's max refresh rate is insufficient as FPS limiters aren't exact and can slightly overshoot the cap due to frametime variances, causing VRR to rapidly disengage/re-engage, which can also cause flicker. Furthermore, V-Sync will fall back to regular buffered behavior when VRR turns off, increasing input lag. A common rule of thumb is to have a framerate cap at least 3 below the max refresh rate, based on [Blur Busters G-Sync 101](<https://blurbusters.com/gsync/gsync101-input-lag-tests-and-settings/5/>):
> To leave no stone unturned, an “at” FPS, -1 FPS, -2 FPS, and finally -10 FPS limit was tested to prove that even far below -2 FPS, no real improvements can be had. ... a -3 FPS limit is the safest bet, and is my new recommendation.

<div style="text-align: center; font-size: 0;">
  <img src="{{site.baseurl}}/assets/images/vrr_fig_1.png" style="max-height: 270px; max-width: 100%; height: auto; width: auto; object-fit: contain; margin: 0; vertical-align: bottom; display: inline-block;" />
  <div style="text-align: left; font-size: 14px; margin-top: 4px; margin-bottom: 20px;">Fig 1. Visual comparison of sync methods. Scanout is the time it takes to draw a frame on screen (top-to-bottom, left to right). It stays constant with VRR, while the refresh interval changes. Note FPS limiter paces beginning of frame 6 after 5 exceeds the cap. Adapted from <a href="https://arxiv.org/pdf/2306.01691" target="_blank">The Influence of Variable Frame Timing on First-Person Gaming</a></div>
</div>
...
[Blur Busters (2017-2023) - G-SYNC 101](<https://blurbusters.com/gsync/gsync101-input-lag-tests-and-settings/>)
...
[Techless (2025) - You are using G-sync wrong (probably)](<https://www.youtube.com/watch?v=5mWMP96UdGU>)
- LDAT, left of screen center @ 240 Hz in Overwatch 2 & Valorant, 7900X + 4080
- NVCP V-Sync + G-Sync + Reflex ideal VRR config

[FR33THY (2022) - NVIDIA Does It Add Input Lag?](<https://www.youtube.com/watch?v=zv2UTb1BllM>)
...
[FR33THY (2022) - AMD Does It Add Input Lag?](<https://www.youtube.com/watch?v=9l_r5ejfPZc>)
...
[Battle(non)sense (2020) - G-Sync & Reflex Low Latency - How To Use Both](<https://www.youtube.com/watch?v=Gub1bI12ODY>)
- Feature overview, no testing
- G-Sync + In-game V-Sync + Reflex/FPS limiter at least 4 below max refresh rate suggested

[Battle(non)sense (2020) - Get The Most Out Of Your G-Sync Monitor](<https://www.youtube.com/watch?v=YR0vNs0ZdWI>)
- Showcase of G-Sync + FPS limiter + V-Sync On vs. Off tearing in Overwatch

[Battle(non)sense (2019) - NVIDIA G-Sync + Null Setup & Input Lag Results](<https://www.youtube.com/watch?v=OAFuiBTFo5E>)
 - 1000 FPS camera @ 144 Hz in Battlefield V and Overwatch
 - G-Sync + NVCP V-Sync + NVCP Low Latency Mode "Ultra" vs. in-game cap with both low and full GPU 
 - Testing (with 441.08) predates NVCP framerate limiter introduced in 441.87

[Battle(non)sense (2017) - FreeSync vs. G-Sync Delay Analysis](<https://www.youtube.com/watch?v=mVNRNOcLUuA>)
- 1200 FPS high speed camera @ 144 Hz in Overwatch
- G-Sync + V-Sync + in-game FPS limiter (at least 2 below) ideal VRR config
- Freesync + V-Sync + in-game FPS limiter (at least 2 below) ideal VRR config
